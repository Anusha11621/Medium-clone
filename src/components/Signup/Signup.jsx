...
export default class Signup extends Component {
  ...
  posturl = () => {
    ...
    fetch(signupurl, options).then((res) => {
      if (!res.ok) {
        res.json().then((error) => {
  ...
  validation = () => {
    ...
    if (this.props.data.signupemail == "") {
      ...
      !this.props.data.signupemail.includes("@") ||
      !this.props.data.signupemail.includes(".")
    ) {
      ...
      if (this.state.error.errors.email && this.state.error.errors.username) {
        return (
          <p className="text-center text-danger">
            *Email and username {this.state.error.errors.email}
          </p>
        );
      } else if (this.state.error.errors.username) {
        return (
          <p className="text-center text-danger">
            Username {this.state.error.errors.username}
          </p>
        );
      } else if (this.state.error.errors.email) {
        return (
          <p className="text-center text-danger">
            Email {this.state.error.errors.email}
          </p>
        );
      } else {
        return <p></p>;
      ...
      <div className="p-5 mt-3 d-flex  justify-content-center gap-3">
        ...
        <form className="d-flex flex-column  align-items-center mt-5">
          ...
          <p className="text-danger">
            <b>{this.props.data.error.signupusername}</b>
          </p>
          <br></br>
          <input
            id="signupemail"
          ...
          ></input>
          <p className="text-danger">
            <b>{this.props.data.error.signupemail}</b>
          </p>
          <br></br>
          <input
            id="signuppassword"
          ...
          ></input>
          <p className="text-danger">
            <b>{this.props.data.error.signuppassword}</b>
          </p>
